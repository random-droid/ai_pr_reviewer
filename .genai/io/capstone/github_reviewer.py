...
def main():
    ...
    ai = ChatGPT(vars_main.chat_gpt_token, vars_main.chat_gpt_model)
    ...
    changed_files = Git.get_diff_files(remote_name=remote_name, head_ref=vars_main.head_ref, base_ref=vars_main.base_ref)
    ...
    for file in changed_files:
        ...
        file_diffs = Git.get_diff_in_file(remote_name=remote_name, head_ref=vars_main.head_ref, base_ref=vars_main.base_ref,
                                          file_path=file)
        if len(file_diffs) == 0:
        ...
        if AiBot.is_no_issues_text(response):
            ...
            for response in responses:
                ...
                if not result:
                    raise GitHubError("Failed to post any comments.")
