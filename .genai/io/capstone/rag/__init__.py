...
How RAG works:
1. Indexing: Walk repo → chunk code at function level → embed with Gemini/OpenAI → store in FAISS
...
Embedding providers:
- GeminiEmbeddings: Free tier available, uses gemini-embedding-001
- OpenAIEmbeddings: Requires paid API key, uses text-embedding-3-small
"""

from .embeddings import OpenAIEmbeddings, GeminiEmbeddings, EmbeddingsBase
